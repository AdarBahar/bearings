...
- **Plain-English Q&A** — `/bearings-ops how do I restart X?` answers from the notes; `/bearings-ops what's running on my server?` verifies live before answering.

## Install

```bash
git clone https://github.com/AdarBahar/bearings && cd bearings
...
```bash
claude "/bearings-doc"                    # document/refresh the repo you're in
claude "/bearings-ops <any question>"     # ask about projects, services, machines
python3 scripts/bearings_sweep.py --force # sweep now (respects refresh_mode; --report to only report)
scripts/install-schedule.sh --uninstall  # remove the timer
```

## Security model

- **No secret values, ever.** Notes record env-file locations and variable names/purposes. `.env` files are never read (only `.env.example`). Your vault syncs wherever your vault syncs — Bearings assumes that's semi-public.
...
```
skills/bearings-setup/   onboarding wizard (interactive)
skills/bearings-doc/     note generator/refresher (local + ssh remotes)
skills/bearings-ops/     plain-English Q&A over vault + live state
scripts/bearings_sweep.py       change detection, dashboard, cost tracking (stdlib only)
